...
-(void) viewWillAppear:(BOOL)animated;
{
    ...
    NSString *mainTable=@"CAO_collegeLevelGenData";
        ...
        if(sqlite3_prepare(db, [qsql_1 UTF8String],-1, &statement_1,nil) == SQLITE_OK )
        {
            ...
            while(sqlite3_step(statement_1) == SQLITE_ROW)
            {
                ...
                if([field5Str isEqualToString:@""])
                    ...
                    newStr = [field5Str substringWithRange:NSMakeRange(3, [field5Str length]-3)];
...
-(IBAction)namePressed:(UIButton *)sender
{
    ...
    Interview=[self getThirdExtraInfoFromTableNamed : @"CAO_collegeLevelGenData" sortBy : name];
    testInterviewPortfolio=[self getFourthExtraInfoFromTableNamed : @"CAO_collegeLevelGenData" sortBy : name];
...
-(NSString *) filePath;
{
    ...
    NSString *dbPath = [[[NSBundle mainBundle] resourcePath ]stringByAppendingPathComponent:@"caoReal_level8Data.sql"];
